...
def show_intensity_3D_plotly_scatter(I, x, y, z, D, equatorial_line=None):
    log_I = np.log10(np.maximum(I, 1e-300))
    vmin = np.min(log_I)
    vmax = np.max(log_I)
    ...
    # Create a circle at angle theta_i from z-axis
    radius = 1.001 * np.sqrt(np.max(x**2 + y**2 + z**2))  # Use the max data extent
    phi = np.linspace(0, 2*np.pi, 100)

    # Calculate points on the circle
    for theta in equatorial_line:
        circle_x = radius * np.sin(theta) * np.cos(phi)
        circle_y = radius * np.sin(theta) * np.sin(phi)
        circle_z = radius * np.cos(theta) * np.ones_like(phi)

        # Add the equatorial line to the figure
